...
project = myproject");
        ...
        [Fact]
        public async Task HookForbiddenForSomeScmTypes()
        {
            ...
            await ApplicationManager.RunAsync(appName, async appManager =>
            {
                ...
                var statusCodes = await Task<int>.WhenAll(
        ...
        private async Task<HttpStatusCode> EnsureDeployDisabled(ApplicationManager appManager, string scmType, string userAgent, string payload)
        {
            ...
            HttpResponseMessage response = await client.PostAsync("deploy", new FormUrlEncodedContent(post));
